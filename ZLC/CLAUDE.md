...
1. **Geometry.** Every airspace appendix in every one of these documents is a chart image with no text layer. Sector boundaries, gates (Antelope, Keyhole, Jepson, Avon, Bird, the Ellis Corridor), the P2/P3/P4 areas and the Heber Shelf line are all names without shapes. Recovering them means carving the images out of the PDFs and measuring them, the way `claude_M98_DDA_Geometry.md` was produced. **Nothing spatial can be built until this is done.**
