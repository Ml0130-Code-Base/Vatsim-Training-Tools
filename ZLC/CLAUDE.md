...
1. **Geometry.** Every airspace appendix in every one of these documents is a chart image with no text layer. Sector boundaries, gates (Antelope, Keyhole, Jepson, Avon, Bird, the Ellis Corridor), the P2/P3/P4 areas and the Heber Shelf line are all names without shapes **in the document set**.
   ...
   **A source outside the documents now covers the sector boundaries (2026-09-02).** The VATGlasses ZLC dataset carries lateral polygons with runway-dependent altitude blocks for all nine S56 sectors and all three Big Sky sectors, plus their ownership chains — see `../claude_Community_Geometry_Sources.md` for the format, the verification and the licence terms. It confirmed all twenty ZLC frequencies in `_shared/claude_ZLC_Positions_Reference.md`, which is why it is trusted this far. **It is community data, not a facility document**: it is second-tier, it must be labelled as such wherever it is drawn, and an owner-supplied boundary supersedes it. It is also **CC BY-NC-SA 4.0**, so copying it into a tool is an owner decision — nothing has been vendored.
